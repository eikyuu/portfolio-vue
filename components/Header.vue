    ...
    <header class="shadow-md fixed top-0 left-0 right-0 z-50 backdrop-blur-xl">
      <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
          ...
          <div class="-mr-2 flex md:hidden">
            <button @click="isOpen = !isOpen" type="button" class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500">
      ...
      <div v-if="isOpen" class="md:hidden">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <ul>
                <li v-for="item in nav" :key="item.to" class="hover:bg-gray-50 text-gray-700 block px-3 py-2 rounded-md text-base font-medium">
                    <NuxtLink :to="item.to">{{ item.label }}</NuxtLink>
                </li>
            </ul>
        </div>
